...
inline void heat_transfer_calculation(uint tid, uint size, uint start, uint end,
	std::vector<double>& jobs_time_taken, CustomBarrier& barrier, TemperatureArray* T, uint steps) {
...
void heat_transfer_calculation_parallel(uint size, uint number_of_threads, TemperatureArray* T, uint steps) {
  ...
  std::vector<double> jobs_time_taken(number_of_threads, 0.0);
	CustomBarrier barrier{static_cast<int>(number_of_threads)};
  ...
  for (unsigned int i = 0; i < number_of_threads; i++) {
    threads.emplace_back(heat_transfer_calculation, i, size, startx[i], endx[i],
				std::ref(jobs_time_taken), std::ref(barrier), T, steps);
  }
  ...
  for (int i = 0; i < number_of_threads; ++i) {
    std::cout << i << ", " << startx[i] << ", " << endx[i] << ", "
    		<< std::setprecision(TIME_PRECISION) << jobs_time_taken[i] << "\n";
  }
